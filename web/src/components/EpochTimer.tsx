...
export function EpochTimer() {
  ...
  return (
    <div className="terminal-accent rounded p-6 md:p-8 relative overflow-hidden group">
      <div
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div className="text-accent/60 text-[11px] tracking-[0.4em] font-bold">
          EPOCH #{currentEpoch.toString()}
        </div>
        <div
          className={`font-mono text-4xl md:text-5xl font-bold tabular-nums tracking-wider ${isUrgent ? "text-dead text-glow" : "text-alive text-glow"}`}
        >
          {mm}:{ss}
          <span className="animate-blink text-accent/40 ml-1">_</span>
        </div>
      </div>
      <div className="relative z-10 h-1.5 bg-accent/10 rounded-full overflow-hidden shadow-inner">
        <div
          className={`h-full rounded-full transition-all duration-1000 ${isUrgent ? "bg-dead shadow-[0_0_20px_rgba(255,106,140,0.8)]" : "bg-alive shadow-[0_0_20px_rgba(109,245,216,0.6)]"}`}
